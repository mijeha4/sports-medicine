...
### Вариант 1: База данных в Docker + приложение локально (Рекомендуемый)

1. **Убедитесь, что Docker и Java 17+ установлены** на вашем компьютере
2. **Скачайте проект** или перейдите в директорию проекта
3. **Запустите базу данных PostgreSQL через Docker:**
   ```bash
   docker-compose up -d db
   ```
4. **Дождитесь запуска базы данных** (несколько секунд)
5. **Запустите приложение Spring Boot локально:**
   ```bash
   mvn spring-boot:run
   ```
6. **Дождитесь завершения сборки и запуска приложения** (может занять 1-2 минуты)
7. **Откройте браузер** и перейдите по адресу: `http://localhost:8080`

### Вариант 2: Полностью локальный запуск
...
### Редактирование атлета

1. Выберите атлета в таблице (кликните по строке)
2. Нажмите кнопку **"Изменить атлета"**
3. Внесите необходимые изменения в открывшейся форме
4. Нажмите **"Save"** для сохранения изменений или **"Cancel"** для отмены
...
1. Выберите врача в таблице (кликните по строке)
2. Нажмите кнопку **"Изменить доктора"**
3. Внесите необходимые изменения в открывшейся форме
4. Нажмите **"Save"** для сохранения изменений или **"Cancel"** для отмены
...
2. **Графики:**
   ...
   - **Столбчатая диаграмма** - Распределение по видам спорта
...
**Решение:**
1. Проверьте, запущен ли Docker и запущена ли БД: `docker-compose ps`
2. Убедитесь, что порт 8080 свободен (приложение) и 5432 (БД)
3. Проверьте логи базы данных: `docker-compose logs db`
4. Если БД не запущена, запустите её: `docker-compose up -d db`
5. Проверьте логи приложения в терминале, где запущен `mvn spring-boot:run`
6. Убедитесь, что Java 17 установлена: `java -version`
...
A: Да, редактирование предусмотрено для всех основных сущностей (атлеты, врачи, медицинские осмотры). Выберите запись в таблице и нажмите кнопку "Изменить" для внесения изменений.
